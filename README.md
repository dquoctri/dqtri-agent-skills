...
Reusable SDLC skills and project memory for coding agents.

`dqtri-agent-skills` is a zero-install SDLC skill and memory system for coding agents. Projects reference shared reusable skills from `~/dqtri-agent-skills/` while keeping durable context in local `./ai-memory/`.

It provides reusable developer workflows, shared rules, and project-local memory to reduce context loss, save tokens, and speed up development.
...
```text
./ai-memory/skill-improvements.md
```

This file is a proposal queue. Entries are not automatically promoted to shared skills.
